import { ExternalLink, Link, Send, Share2 } from 'lucide-react'
              ...
              selectedProposal.sources.map((source, index) => (
                <div key={`${source.url ?? source.title ?? 'source'}-${index}`} className="text-sm text-zinc-200">
                  ...
                  <p className="mt-1 text-zinc-400">
                    Medio: <span className="text-zinc-200">{readText(source.sourceName ?? source.mediaName)}</span>
                  </p>
                  ...
                  {source.url ? (
                    <a
                      ...
                      target="_blank"
                      rel="noreferrer"
                      className="mt-1 inline-flex min-w-0 items-start gap-1 break-all text-cyan-200 hover:text-cyan-100"
                    >
